...
void Util::DrawLine(glm::vec2 start, glm::vec2 end, glm::vec4 colour)
{
	...
	int g = floor(colour.g * 0.0f);
	int b = floor(colour.b * 0.0f);*/
	int a = floor(colour.a * 1.0f);

	const auto renderer = /* TheGame::Instance()->getRenderer()*/ Renderer::Instance()->getRenderer();

	SDL_SetRenderDrawColor(renderer,255.0, 255.0, 255.0, a);
	SDL_RenderDrawLine(renderer, start.x, start.y, end.x, end.y);
	SDL_RenderDrawLine(renderer, start.x, start.y-1, end.x, end.y-1);
